  ...
  Implements: Options,
	...
	initialize: function(obj, options) {
		...
		for (var i in obj) {
			...
			// create cal button
			cal.button.addClass(this.classes.calendar+'But').addEvent('click', function(cal) { this.toggle(cal); }.pass(cal, this)).injectAfter(cal.el);
	...
	element: function(el, f, cal) {
		...
		if (el.get('tag') == 'select') { // select elements allow the user to manually set the date via select option
			el.addEvent('change', function(cal) { this.changed(cal); alert(this.options.begins); }.pass(cal, this));
		}
